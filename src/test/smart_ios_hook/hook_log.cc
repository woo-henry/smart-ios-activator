...
int InitLog()
{
	...
	do
	{
		...
		if (log_path == nullptr)
		{
			MessageBox(nullptr, TEXT("SmartMemAlloc Error"), TEXT("Hook"), MB_OK);
			break;
		}

		if (!SmartFsGetAppPath(log_path, TEXT("log")))
		{
			MessageBox(nullptr, TEXT("SmartFsGetAppPath Error"), TEXT("Hook"), MB_OK);
			break;
		}

		if (!SmartLogInitialize(log_path, TEXT("hook.log")))
		{
			MessageBox(nullptr, TEXT("SmartLogInitialize Error"), TEXT("Hook"), MB_OK);
