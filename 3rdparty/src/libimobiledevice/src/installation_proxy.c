...
static instproxy_error_t instproxy_receive_status_loop(instproxy_client_t client, plist_t command, instproxy_status_cb_t status_cb, void *user_data)
{
	...
	do {
		...
		if (node) {
			...
			if (res != INSTPROXY_E_SUCCESS) {
				debug_info("command: %s, error %d, code 0x%08llx, name: %s, description: \"%s\"", command_name, res, error_code, error_name, error_description ? error_description: "N/A");
				complete = 1;
			}
...
LIBIMOBILEDEVICE_API_MSC void instproxy_free(char** value)
{
	if (value == NULL)
		return;
