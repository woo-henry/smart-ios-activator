...
static afc_error_t afc_dispatch_packet(afc_client_t client, uint64_t operation, uint32_t data_length, const char* payload, uint32_t payload_length, uint32_t *bytes_sent)
{
	...
	*bytes_sent = 0;
	...
	sent = 0;
	service_send(client->parent, (const char*)client->afc_packet, sizeof(AFCPacket) + data_length, &sent);
...
LIBIMOBILEDEVICE_API_MSC void afc_free_file_info(char*** file_information)
{
	if (file_information == NULL)
		return;

	char** strs = *file_information;
		return;
