...
int socket_create(const char* addr, uint16_t port)
{
	...
	for (rp = result; rp != NULL; rp = rp->ai_next) {
		...
		if (rp->ai_family == AF_INET6) {
			if (setsockopt(sfd, IPPROTO_IPV6, IPV6_V6ONLY, (const char*)&yes, sizeof(int)) == -1) {
...
static void freeifaddrs(struct ifaddrs *ifa)
{
	...
	mi_free(ifa->ifa_name);
	mi_free(ifa->ifa_addr);
	mi_free(ifa->ifa_netmask);
	mi_free(ifa->ifa_dstaddr);
...
static int getifaddrs(struct ifaddrs** ifap)
{
	...
	*ifap = NULL;
	...
	do {
		pAddresses = (IP_ADAPTER_ADDRESSES*)mi_malloc(outBufLen);
		if (pAddresses == NULL) {
	...
	for (adapter = pAddresses; adapter; adapter = adapter->Next) {
		...
		for (IP_ADAPTER_UNICAST_ADDRESS *unicast = adapter->FirstUnicastAddress; unicast; unicast = unicast->Next, ++unicastIndex) {
			...
			if (!ifa) {
				ifa = mi_malloc(sizeof(struct ifaddrs));
				if (!ifa) {
					errno = ENOMEM;
					mi_free(pAddresses);
					return -1;
				...
				*ifap = ifa;
				...
				struct ifaddrs* ifanew = mi_malloc(sizeof(struct ifaddrs));
				if (!ifanew) {
					freeifaddrs(*ifap);
					mi_free(pAddresses);
					errno = ENOMEM;
			...
			/* name */
			ifa->ifa_name = mi_strdup(adapter->AdapterName);
			...
			/* address */
			ifa->ifa_addr = (struct sockaddr*)mi_malloc(sizeof(struct sockaddr_storage));
			memcpy(ifa->ifa_addr, unicast->Address.lpSockaddr, unicast->Address.iSockaddrLength);

			/* netmask */
			ifa->ifa_netmask = (struct sockaddr*)mi_malloc(sizeof(struct sockaddr_storage));
...
#ifdef SO_NOSIGPIPE
	if (setsockopt(sfd, SOL_SOCKET, SO_NOSIGPIPE, (const char*)&yes, sizeof(int)) == -1) {
		perror("setsockopt()");
		socket_close(sfd);
		return -1;
	}
	...
	do {
...
#endif
		{
			...
			if (select(sfd + 1, NULL, &fds, NULL, &timeout) == 1) {
				int so_error;
				socklen_t len = sizeof(so_error);
				getsockopt(sfd, SOL_SOCKET, SO_ERROR, (char*)&so_error, &len);
				if (so_error == 0) {
					errno = 0;
					break;
				}
...
int socket_send(int fd, void *data, size_t length)
{
	...
	flags |= MSG_NOSIGNAL;
#endif
	return send(fd, (char*)data, length, flags);
}
